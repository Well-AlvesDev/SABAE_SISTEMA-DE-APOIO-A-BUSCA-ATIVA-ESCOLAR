...
/**
 * Verifica se existe uma chamada registrada no Supabase usando auth nativo
 ...
 */
async function verificarChamadaNoSupabaseNativo(turma, dia, mes) {
    try {
        console.log(`🔍 Verificando chamada no Supabase para ${turma} - ${dia}/${mes} (AUTH NATIVO)...`);
        ...
        const { data, error } = await supabaseClient.rpc('obter_chamada_existente_nativa', {
...
/**
 * Busca uma chamada completa do Supabase e retorna pronta para edição (AUTH NATIVO)
 * @param {string} turma - Nome da turma/sala
 * @param {number} dia - Dia do mês (1-31)
 * @param {string} mes - Nome do mês
 * @returns {Promise<Object>} { sucesso, chamada, prontaParaEditar }
 */
async function buscarChamadaSupabaseParaEditarNativo(turma, dia, mes) {
    try {
        const resultado = await verificarChamadaNoSupabaseNativo(turma, dia, obterNumeroMes(mes));
...
/**
 * @deprecated Use verificarChamadaNoSupabaseNativo() em vez desta função
 */
async function verificarChamadaNoSupabase(turma, dia, mes) {
    console.warn('⚠️ verificarChamadaNoSupabase() foi depreciada. Use verificarChamadaNoSupabaseNativo()');
    try {
...
/**
 * @deprecated Use buscarChamadaSupabaseParaEditarNativo() em vez desta função
 */
async function buscarChamadaSupabaseParaEditar(turma, dia, mes) {
    console.warn('⚠️ buscarChamadaSupabaseParaEditar() foi depreciada. Use buscarChamadaSupabaseParaEditarNativo()');
    try {
